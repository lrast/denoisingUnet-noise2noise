...
def runBasicTraining(model, targetdir, group=None, max_epochs=1000):
    ...
    shutil.copyfile(checkpoint_callback.best_model_path,
                    f'lightning_logs/{targetdir}/best.ckpt')
...
def numbersSweep():
    ...
    def initAndTrain():
        ...
        targetdir = f'groundTruthSweep/N{N}M{M}nr{nr}rep{rep}'

        checkpoint_callback = ModelCheckpoint(
                                monitor='val_loss',
                                dirpath='lightning_logs/'+targetdir, 
                                every_n_epochs=1, save_top_k=1
                                )
        earlystop_callback = EarlyStopping(
                                monitor='val_loss', mode='min',
